...
- (void)processResultsOfBatchRequest:(FOSWebServiceRequest *)batchRequest
    ...
    for (NSUInteger i = 0; i < requests.count; i++) {
        ...
        if (jsonSuccess != nil) {
            [nextRequest setJsonResult:jsonSuccess];
        }
        else {
            NSError *error = nil;
...
@"\n"
     ...
     @"              ATTRIBUTES :: ALL_EXCEPT  ( 'objectId', 'emailVerified', 'sessionToken' ), \n"
    @"           READ_ONLY_ATTRIBUTE { $ATTRDESC.name : $ATTRDESC.name } \n"
...
@"\n"
     ...
     @"              { ( $RELDESC.name + '.className' ) : $RELDESC.destinationEntity.name }, \n"
     @"              { ( $RELDESC.name + '.objectId' ) : $CMO.`$RELDESC.destinationEntity.name.lowercaseString`.jsonIdValue } ;\n"
     @"            JSON_ID_BINDING :: ( $RELDESC.name + '.objectId' ) ;\n"
     @"            RELATIONSHIPS :: ALL_EXCEPT ( 'user' ) ;\n"
     @"            ENTITIES :: ALL ; \n"
     @"          , \n"
     @"          RELATIONSHIP_BINDING :: \n"
     @"            JSON_BINDING :: \n"
     @"              { ( $RELDESC.name + '.__type' ) : 'Pointer' }, \n"
     @"              { ( $RELDESC.name + '.className' ) : '_User' }, \n"
     @"              { ( $RELDESC.name + '.objectId' ) : $CMO.`$RELDESC.destinationEntity.name.lowercaseString`.jsonIdValue } ;\n"
     @"            JSON_ID_BINDING :: ( $RELDESC.name + '.objectId' ) ;\n"
     @"            RELATIONSHIPS :: ( 'user' ) ;\n"
     @"            ENTITIES :: ALL ; \n"
